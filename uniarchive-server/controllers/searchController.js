...
const searchResearch = async (req, res) => {
  ...
  try {
    let query = `SELECT r.research_id, r.title, r.abstract, r.academic_year, r.category, r.file_url, 
    GROUP_CONCAT(a.full_name) AS authors 
    FROM research r 
    LEFT JOIN research_authors ra ON r.research_id = ra.research_id 
    LEFT JOIN author a ON ra.author_id = a.author_id WHERE 1=1`
    const params = []
...
const viewResearch = async (req, res) => {
  const { id } = req.params
  try {
    const [rows] = await pool.query(`SELECT r.*, GROUP_CONCAT(a.full_name) 
      AS authors FROM research r 
      LEFT JOIN research_authors ra ON r.research_id = ra.research_id 
      LEFT JOIN author a ON ra.author_id = a.author_id 
      WHERE r.research_id = ${parseInt(id)} GROUP BY r.research_id`)
